...
def get_sensor():
       ...
       print(temp, bpm, spo2)
       oled_init()
       oled_write(draw,0,3,str(temp))
       oled_write(draw,0,4,str(bpm))
       oled_write(draw,0,5,str(spo2))
...
GPIO.setup(back, GPIO.IN, pull_up_down=GPIO.PUD_UP) # Set pin 10 to be an in>
GPIO.setup(next, GPIO.IN, pull_up_down=GPIO.PUD_UP)
GPIO.setup(end, GPIO.IN, pull_up_down=GPIO.PUD_UP)

GPIO.add_event_detect(back,GPIO.RISING,callback=welcome) # Setup event on pi>
...
oled_write(draw, 0, 0, "Hello")
oled_write(draw, 0, 1, data[0])
oled_write(draw, 0, 2, data[1])
